            ...
            <div class="" id="seller-fixed">
                <div class="dis-con">
                    ...
                    $form = ActiveForm::begin([
                        'id' => 'add-seller-form',
                        'action'=>Yii::$app->urlManager->createUrl([\common\widgets\Variable::$editBrand_url,'id'=>Tool::echoEncodeString($customerModel->id)]),
                        'method'=>'post'
                    ]);
                    ?>
                    ...
                    <div class="row margin0">
                        <?=$form->field($model, 'parentId', ['options' => ['class' => 'form-group col-lg-3 paddingLeft']])->textInput(['disabled'=>true])->label('所属客户分类');?>
                    </div>
                    <?= $form->field($model, 'blogo')->label('品牌logo（灰色）')->widget('common\widgets\file_upload\FileUpload',[
                        'config'=>[]
                    ]) ?>
                    <?= $form->field($model, 'clogo')->label('品牌logo（彩色）')->widget('common\widgets\file_upload\FileUpload',[
                        'config'=>[]
                    ]) ?>
                    <?php
